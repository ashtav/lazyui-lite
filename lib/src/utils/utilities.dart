...
Try to check [$member]''';
  ...
  /// Example usage:
  /// ```dart
  /// int randomInt = Utils.getRange(1, 10);
  /// double randomDouble = Utils.getRange(1.0, 10.0);
  /// ```
  static T getRange<T>(T min, T max) {
    if (min is int && max is int) {
      return math.Random().nextInt(max - min) + min as T;
    } else if (min is double && max is double) {
      return math.Random().nextDouble() * (max - min) + min as T;
    } else {
      return min;
    }
  }
  ...
  /// - Replaces the leading `0` with the given [prefix].
  /// - Ensures the phone number starts with the specified [prefix].
  ///
  /// [phone]: The input phone number (nullable).
  /// [prefix]: The international prefix to use, defaults to `'62'`.
  ///
  /// Returns: The formatted phone number as a string.
  static String prefixPhone(String? phone, {String prefix = '62'}) {
    final cleanedPhone = phone?.replaceAll(RegExp(r"[^\d]"), '') ?? '';
    if (cleanedPhone.startsWith("0")) {
      return "$prefix${cleanedPhone.substring(1)}";
    }
    return cleanedPhone.startsWith(prefix)
        ? cleanedPhone
        : "$prefix$cleanedPhone";
  }
}
